...
export function IssuerDashboard({ onNavigateToVerify }: { onNavigateToVerify: (id: string) => void }) {
  ...
  const [apiKey, setApiKey] = useState(
    localStorage.getItem("passapawn:verifier_api_key") ??
      localStorage.getItem("passapawn:policy_admin_api_key") ??
      localStorage.getItem("passapawn:admin_api_key") ??
      "",
  );
  ...
  useEffect(() => {
    localStorage.setItem("passapawn:verifier_api_key", apiKey);
  }, [apiKey]);
  ...
  const revokeOnChain = async (recordId: string) => {
    const domainCapId = localStorage.getItem("passapawn:last_domain_cap_id") ?? "";
    if (!domainCapId.trim()) {
  ...
  const hasDomainCap = Boolean((localStorage.getItem("passapawn:last_domain_cap_id") ?? "").trim());
  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900 p-6 shadow-md">
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        <p className="text-xs text-gray-400">Showing {displayedRows.length} of {rows.length} credentials</p>
      ...
      {displayedRows.length === 0 ? (
        <p className="mt-3 text-sm text-gray-400">No credentials issued by this domain yet.</p>
      ) : (
              ...
              {displayedRows.map((row) => {
                ...
                return (
                  <tr key={row.record_id} className="border-t border-gray-800">
                    ...
                    <td className="text-gray-200">{md?.credential_type ?? "Credential"}</td>
                      ...
                      <div className="flex gap-2">
                        ...
                        {hasDomainCap && row.verdict.status !== "revoked_on_chain" && (
                          <button
                            ...
                            onClick={() => void revokeOnChain(row.record_id)}
