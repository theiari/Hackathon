...
async function run(): Promise<void> {
  ...
  const school = useRealTx
    ? await createDomainAndTemplateReal(client, "PassaPawn Demo School", "B2 Language Certificate", schoolFields)
    : await createDomainAndTemplateMock("school", "b2cert");

  const clinic = useRealTx
    ? await createDomainAndTemplateReal(client, "PassaPawn Demo Clinic", "Vaccination Record", clinicFields)
